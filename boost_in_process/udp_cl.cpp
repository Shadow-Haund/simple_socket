...
class udp_client{
    public:
        udp_client(io_c &io, int use_hz, int hz, std::string serv_ip, int serv_port, std::string msg)
...
int main(int argc, char* argv[]){
    ...
    udp_client cl(io_context, param_val->use_hz, param_val->hz, param_val->serv_ip, param_val->serv_port, param_val->msg);
    cl.start();
    io_context.run();
    return 0;
}